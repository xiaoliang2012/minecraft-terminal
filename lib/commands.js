...
commands.follow = async (player, range) => {
	...
	while (commands.tmp.botMoving === true) {
		...
		const dist = distance(bot.entity?.position, bot.players[player].entity.position);
		if (dist < range) {
			await sleep(150);
			bot.setControlState('forward', false);
			continue;
		}
		bot.lookAt(bot.players[player].entity.position.offset(0, playerHeight, 0), true);
		bot.setControlState('forward', true);
		await sleep(80);
	}
	bot.setControlState('sprint', sprintState);
...
commands.optcmd = async (inp) => {
	...
	const inputCommand = inp[0].toLowerCase();

...
commands.inventory = async (id, action, ...slots) => {
	...
	if (action === undefined) {
		...
		const a = (items) => {
			...
			let make = (item, itemName) => {
				out = `${out}\n   | #${item.slot}: x${item.count}  ${itemName}`;
			};
			if (isWindow === false) {
				make = (item, itemName) => {
					out = `${out}\n ${item.slot - bot.inventory.hotbarStart} | #${item.slot}: x${item.count}  ${itemName}`;
				};
			}
			let out = '';
			const itemm = items?.[0];
			...
			if (isWindow === false) {
				out = ` ${itemm.slot - bot.inventory.hotbarStart} | #${itemm.slot}: x${itemm.count}  ${itemNamee}`;
			} else {
				out = `${out}   | #${itemm.slot}: x${itemm.count}  ${itemNamee}`;
			};
	...
	if (action === 'move' && !isNaN(slots[0]) && !isNaN(slots[1])) {
		...
		await bot.clickWindow(slots[0], 0, 0);
		await bot.sleep(100);
		await bot.clickWindow(slots[1], 0, 0);
		return;
	}

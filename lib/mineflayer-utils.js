...
async function placeBlock (x, y, z) {
	...
	for (let a = 0, b = 0; a < 6; a++) {
		...
		const adjBlockPos = blockPos.offset(offset.x, offset.y, offset.z);
	...
	for (let i = 0; i < adjBlocks.length; i++) {
		...
		await sleep(50);
		try {
			await bot.placeBlock(adjBlocks[i], blockID2Vec[blockRef.face]);
		} catch (err) {
			done();
...
async function move (x, z, range = 0, timeout = Infinity) {
	if ([

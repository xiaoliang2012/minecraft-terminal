...
commands.follow = async (matchesStr, range) => {
	...
	const usage = 'Usage: .follow <Conditions:$name=pig|$name!=pig|...> <Range>. Range > 0';
...
commands.smartFollow = async (matchesStr, range) => {
	...
	const usage = 'Usage: .smartFollow <Conditions:$name=pig|$name!=pig|...> <Range>. Range > 0';
	if (!['string', 'boolean'].includes(typeof matchesStr) || isNaN(range) || range <= 0) {
		info(usage);
		return;
	}
...
commands.attack = (matchesStr, cps, reach, minreach) => {
	...
	const usage = 'Usage: .attack <Conditions:$name=pig|$name!=pig|...> <CPS> <MaxReach> <MinReach>. MaxReach > MinReach, CPS > 0';

...
commands.move = (direction, distance) => {
	...
	if (direction && (distance === undefined || distance > 0)) {
		if (!bot.pathfinder.isMoving() || !commands.tmp.botMoving) {
			...
			bot.pathfinder.goto(new GoalXZ(position.x + x, position.z + z));
		}
	...
	if (commands.tmp.botMoving === false) {
		if (player && range > 1) {
			...
			while (commands.tmp.botMoving === true) {
				if (bot.players[player]?.entity?.position) {
					if (distance(bot.players[player]?.entity?.position, bot.entity.position) > range) {
						try {
							time = Date.now();
							goal = new GoalNear(bot.players[player].entity.position.x, bot.players[player].entity.position.y, bot.players[player].entity.position.z, range);
							await bot.pathfinder.goto(goal);
						} catch (err) {
							error(`${err.message} Retrying in 0.2 seconds`);
							await sleep(200);
						}
						if (Date.now() - time < 95) {
							await sleep(100);

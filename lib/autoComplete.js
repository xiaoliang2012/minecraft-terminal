...
function getCompletion (rlInterface, completionsObj, options = { minLength: 1, startOnly: false, caseInsensitive: false }) {
	...
	for (let a = 0; a < inputWords.length; a++) {
		...
		for (let b = 0; b < wordsToBeCompleted.length; b++) {
			...
			if ((options.caseInsensitive === true && word.toLowerCase() === value.slice(0, word.length).toLowerCase()) || (word === value.slice(0, word.length))) {
				match = value;
				doesMatch = true;
				break;
			}
	...
	process.stdin.on('data', (data) => {
		if (data.toString() === '\t') {
			...
			if (rlInterface.line.length === rlInterface.cursor) {
				...
				const toComplete = getCompletion(rlInterface, completionsObj, options);
				if (toComplete) {
		...
		if (rlInterface.line.length === rlInterface.cursor) {
			suggestCompletion(completionsObj, options);
		}
	});
}

...
commands.follow = async (player, range) => {
	if (commands.tmp.botMoving === false) {
		if (player && range > 1) {
			...
			let goal = new GoalFollow(bot.players[player].entity, range);
			goal.rangeSq = range;
			let notFound = false;
			while (commands.tmp.botMoving) {
				...
				if (notFound) {
					notFound = false;
					goal = new GoalFollow(bot.players[player].entity, range);
					goal.rangeSq = range;
				}
...
commands.distance = (x1, y1, z1, x2, y2, z2) => {
	...
	info(`Distance: ${distance(point1, point2).toFixed(2)}`);
};

commands.position = () => {
	const pos = bot.entity.position;
	info(`Position: ${Number(pos.x.toFixed(2))}, ${Number(pos.y.toFixed(2))}, ${Number(pos.z.toFixed(2))}`);
};
...
commands.optcmd = (inp, inps) => {
	...
	case 'dig': commands.dig(inp[1], inp[2], inp[3]); break;
...
commands.lookAt = (who, reach, minreach, force) => {
	if (who && reach > 0 && (minreach === undefined || reach > minreach)) {
		...
		if (commands.tmp.botAttacking || commands.tmp.botLooking) {
...
commands.cmd = (msg) => {
	...
	if (msg.match(/(?<=^\.)./)) {
		const tmp = parseCoords(msg.match(/(?<=^\.).+/)[0], bot.entity.position);
		const inps = [];
		inps[0] = tmp;

 ...
 *
 */
function botAttack (reach, minreach, maxreach, entity) {
	if (!entity?.position) {
		return;
	}
...
commands.attack = (match, equals, value, cps, reach, minreach) => {
	...
	if (!match || !value || !equals || isNaN(cps) || isNaN(reach) || isNaN(minreach) || cps <= 0 || reach <= 0 || reach < minreach) {
		info('Usage: .attack <Match> <Equals> <Value> <CPS> <MaxReach> <MinReach>. MaxReach > MinReach, CPS > 0');
		return;
	}

	commands.tmp.botLooking = true;
	commands.tmp.botAttacking = true;
	success(`Attacking nearest entity if ${match}='${value}' with ${cps}CPS if MinReach(${minreach}) < distance < MaxReach(${reach})`);
	const matchEntity = (entity, match, value, equals) => {
		if (equals === '!=' || equals === '!') {
			// eslint-disable-next-line eqeqeq
			return (entity[match]?.toLowerCase?.() || entity[match]) != (value.toLowerCase?.() || value);
		}
		// eslint-disable-next-line eqeqeq
		return (entity[match]?.toLowerCase?.() || entity[match]) == (value.toLowerCase?.() || value);
	};
		...
		if (commands.tmp.botAttacking === true) {
			botAttack(
				...
				bot.nearestEntity((entity) => {
					return matchEntity(entity, match, value, equals);
				})
			);

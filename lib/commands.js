...
// commands.test = (id, action, slot, slot2) => {
// 	if (id !== 'inventory' || id !== 0 || id !== 'container' || id !== 1) {
// 		// CHANGE THIS
...
commands.optcmd = async (inp, options) => {
	...
	const aliasKeys = toLowerCaseArr(Object.keys(config.commands.commandAliases));
	const aliasValues = toLowerCaseArr(Object.values(config.commands.commandAliases));
	...
	if (config.commands.enableNonVanillaCMD === false && nonVanillaCommands.includes(inputCommand)) {

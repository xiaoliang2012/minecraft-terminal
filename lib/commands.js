...
function getEntityMatches (matchesStr) {
	const matches = [];
	for (let i = 0, p = 0, tmp = matchesStr.split(','); i < tmp?.length; i++) {
		const match = tmp[i];
		if (match.match(/[^=]+(?<!!)=[^=]+/)) {
			const split = match.split('=');
			matches[p] = { key: split[0], val: split[1], eq: true };
			p++;
			continue;
		}
		if (match.match(/[^!=]+!=[^!=]+/)) {
			const split = match.split('!=');
			matches[p] = { key: split[0], val: split[1], eq: false };
			p++;
		}
	}
	return matches;
...
function matchEntity (entity, matches) {
	for (let i = 0; i < matches.length; i++) {
		const match = matches[i];
		const { key, val, eq } = match;

		const mm = (entity[key]?.toLowerCase?.() || entity[key]);
		const dd = (val.toLowerCase?.() || val);
		if (eq === false) {
...
async function botFollow (matches, range) {
	if (!matches || !range) {
		return;
	}
	...
	while (commands.tmp.botMoving === true) {
		...
		const entity = bot.nearestEntity((entity) => {
			return matchEntity(entity, matches);
		});
		if (!entity) {
			await sleep(200);
			continue;
		...
		await withPriority(10, 120, true, botLookPriorityCache, () => {
...
commands.follow = async (matchesStr, range) => {
	...
	if (!matchesStr || !(range > 0)) {
		info('Usage: .follow <Conditions:name=pig|name!=pig|...> <Range>. Range > 0');
		return;
	}
	commands.tmp.botMoving = true;
	let unit;
	...
	const matches = getEntityMatches(matchesStr);
	botFollow(matches, range);
};
...
async function botSmartFollow (matches, range) {
	if (!matches || !range) {
		return;
	}
	let cooldown = 0;
...
commands.smartFollow = async (matchesStr, range) => {
	...
	if (typeof matchesStr !== 'string' || isNaN(range) || range <= 0) {
		info('Usage: .follow <Conditions:name=pig|name!=pig|...> <Range>. Range > 0');
		return;
	}
	let unit;
 ...
 *
 */
async function botAttack (reach, minreach, maxreach, entity) {
	...
	if (dist < reach && dist > minreach) {
		for (let i = 0; i < 2; i++) {
			const ent = bot.entityAtCursor(maxreach);
			if (shallowCompareObj(ent, entity)) {
				bot.attack(ent);
				return;
			}
			await withPriority(3, 100, true, botLookPriorityCache, () => {
...
commands.attack = (matchesStr, cps, reach, minreach) => {
	...
	if (typeof matchesStr !== 'string' || isNaN(cps) || isNaN(reach) || isNaN(minreach) || cps <= 0 || reach <= 0 || reach < minreach) {
		info('Usage: .attack <Conditions:name=pig|name!=pig|...> <CPS> <MaxReach> <MinReach>. MaxReach > MinReach, CPS > 0');
		return;
	}

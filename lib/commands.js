...
commands.follow = async (player, range) => {
	...
	while (commands.tmp.botMoving === true) {
		...
		await withPriority(10, 180, true, botLookPriorityCache, () => {
...
async function botAttack (reach, minreach, maxreach, entity) {
	...
	if (dist < reach && dist > minreach) {
		for (let i = 0; i < 2; i++) {
			...
			await withPriority(3, 100, false, botLookPriorityCache, () => {
...
commands.attack = (matchesStr, cps, reach, minreach) => {
	...
	const matchEntity = (entity, matches) => {
		for (let i = 0; i < matches.length; i++) {
			const match = matches[i];
			const { key, val, eq } = match;

			if (['object', 'orb'].includes(entity.type)) {
				return false;
			}

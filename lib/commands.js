const mineflayerUtils = require('./mineflayer-utils');
...
commands.place = async (x, y, z) => {
	...
	try {
		await mineflayerUtils.placeBlock(x, y, z);
	} catch (err) {
		commands.tmp.botLooking = false;
...
commands.move = async (direction, distance) => {
	...
	// });
	if ((await mineflayerUtils.move(position.x + x, position.z + z, 0, Infinity)) === true) {
...
commands.follow = async (matchesStr, range) => {
	...
	const usage = 'Usage: .follow <EntityMatches:$name=pig|$name!=pig|...> <Range>. Range > 0';
...
commands.smartFollow = async (matchesStr, range) => {
	...
	const usage = 'Usage: .smartfollow <EntityMatches:$name=pig|$name!=pig|...> <Range>. Range > 0';
	if (!['string', 'boolean'].includes(typeof matchesStr) || isNaN(range) || range <= 0) {
		info(usage);
		return;
	}
...
commands.attack = (matchesStr, cps, reach, minreach) => {
	...
	const usage = 'Usage: .attack <EntityMatches:$name=pig|$name!=pig|...> <CPS> <MaxReach> <MinReach>. MaxReach > MinReach, CPS > 0';
...
commands.look = async (a, b, c) => {
	if (a && isNaN(a)) {
		...
		const dir = a;
		if (mineflayerUtils.look(dir) === false) {
...
commands.inventory = async (id, action, ...slots) => {
	...
	if (action === undefined) {
		...
		if (isContainer === true) {
			...
			inventoryItems = mineflayerUtils.getItems(bot.currentWindow);
		} else {
	...
	if (action === 'dropall') {
		let items;
		if (isContainer === true) {
			items = mineflayerUtils.getItems(bot.currentWindow, bot.inventory.inventoryEnd);
		} else {

...
const cursorMove = (row, column) => {
	if ((isNaN(row) && row !== undefined) || (isNaN(column) && column !== undefined)) console.error('Value must be an integer');
...
const cursorTo = (row, column) => {
	if (isNaN(row) || isNaN(column)) console.error('Both values need to be filled with numbers');
...
const cursorBeg = (lines) => {
	if (isNaN(lines)) console.error('Value need to be filled with numbers');
	else {
...
const cursorMoveColumn = (column) => {
	if (isNaN(column)) console.error('Value need to be filled with numbers');
